...
def aes_encode_ecb(text, key='1' * 16):
    """
    aes 加密 ECB/128/PAD5
    text: ( bytes)  dict类型转换为json字符串再进行加密
    key : 16 字节长度的秘钥
    """
    BS = AES.block_size
    # pad = lambda s: s + (BS - len(s) % BS) * chr(BS - len(s) % BS)
    def pad(s):
        # r = s + (BS - len(s) % BS) * chr(BS - len(s) % BS)
        r = s + (BS - len(s) % BS) * bytes( [ BS - len(s) % BS ] )
        return r
...
def make_signature_for_green(secret_key, params):
    ...
    """
    values = list(map(str,list(params.values())))
...
def sign_check_and_get_data(jsondata,keyhex=keyhex,signature='signature' , content='content'):
    ...
    """
    params = {}
    if isinstance(jsondata,(str,bytes)):
...
def sign_data(datadict,keyhex=keyhex,signature='signature' , content='content'):
    ...
    params[content] = datadict.get(content,{})
    # params[content] = datadict
...
def simple_encrpyt(keyhex, data:bytes )->bytes:
    key = binascii.a2b_hex(keyhex)
    text = base64.b64encode(aes_encode_ecb(data, key))
    return text

def simple_decrypt(keyhex,ciphertext):
    key = binascii.a2b_hex(keyhex)
    ciphertext = base64.b64decode(ciphertext)
    data = aes_decode_ecb(ciphertext, key)
    return data
...
# text = aes_decode_ecb(ciphertext,key).decode()
# print(text)
